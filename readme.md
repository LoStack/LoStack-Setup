...
## Directory Structure
LoStack generates, reads, and manages a variety of config files
```
/
│
├── docker-compose.yml
...
| `docker-compose.yml` | From this repo, read by LoStack |
| `lostack-compose.yml` | Semi-automatically managed by LoStack (user edits preserved) |
| `.env` | template.env from this repo, user edited |
| `appdata/lostack/LoStack-Depot/` | Git repo pulled from LoStack/LoStack-Depot |
| `appdata/lostack-db/` | MariaDB config, used by both LoStack and Authelia |
| `certs/_wildcard.DOMAIN.EXT-key.pem` | Wildcard private key (generated by LoStack first run) |
| `certs/_wildcard.DOMAIN.EXT.pem` | Wildcard cert (generated by LoStack first run) |
| `certs/DOMAIN.EXT-key.pem` | Domain private key (generated by LoStack first run) |
| `certs/DOMAIN.EXT.pem` | Domain cert (generated by LoStack first run) |
| `certs/rootCA-key.pem` | Root CA private key (generated by LoStack first run) |
| `certs/rootCA.pem` | Root CA cert (generated by LoStack first run) |
| `config/authelia/lostack_secrets/jwt_secret` | Generated by Authelia first run |
| `config/authelia/lostack_secrets/session_secret` | Generated by Authelia first run |
| `config/authelia/lostack_secrets/storage_encryption_key` | Generated by Authelia first run |
| `config/authelia/configuration.yml` | Generated by LoStack first run |
| `config/coredns/Corefile` | Optional, generated by LoStack first run |
| `config/openldap/` | Generated by OpenLDAP, users managed through LoStack |
| `config/traefik/dynamic.yml` | Generated by LoStack first run, user-managed after |
| `config/traefik/lostack-dynamic.yml` | Managed by LoStack (services/routers for LoStack services) |
| `config/traefik/lostack-routes-dynamic.yml` | Managed by LoStack (services/routers for LoStack routes) |
| `config/lostack/sessions.json` | Managed by LoStack (tracking for autostart API) |
